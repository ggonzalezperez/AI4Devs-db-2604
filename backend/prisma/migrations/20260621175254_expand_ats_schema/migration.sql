...
-- CreateEnum
CREATE TYPE "PositionStatus" AS ENUM ('DRAFT', 'OPEN', 'PAUSED', 'CLOSED', 'ARCHIVED');
...
-- Prisma 5.x no expresa CHECK en el schema, por lo que se añaden a nivel SQL.
-- El rango de la puntuación de entrevista debe estar entre 0 y 100.
ALTER TABLE "Interview" ADD CONSTRAINT "Interview_score_range_chk" CHECK ("score" IS NULL OR ("score" >= 0 AND "score" <= 100));
-- El salario máximo no puede ser inferior al mínimo.
ALTER TABLE "Position" ADD CONSTRAINT "Position_salary_range_chk" CHECK ("salaryMin" IS NULL OR "salaryMax" IS NULL OR "salaryMax" >= "salaryMin");
